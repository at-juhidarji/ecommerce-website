      ...
      <nav className="w-full bg-[#09090b] sticky top-0 z-50 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 h-16 sm:h-20 flex items-center justify-between gap-4">
          ...
          <div className="hidden lg:flex items-center gap-6 text-base font-normal text-white h-full">
            <button
              onClick={() => navigate("/products")} // Use the string path defined in your Routes
              className="hover:text-slate-400 transition"
            >
              On Sale
            </button>
            <button className="hover:text-slate-400 transition">New Arrivals</button>
            ...
            <div className="group h-full flex items-center justify-center">
              <button className="relative flex items-center gap-1 hover:text-slate-400 transition-colors cursor-pointer h-full px-2 group/btn">
                ...
                <span className="absolute bottom-4 left-2 right-2 h-[2px] bg-orange-400 transform scale-x-0 group-hover/btn:scale-x-100 transition-transform duration-300 origin-left" />
              </button>
              ...
              <div className="absolute top-full left-0 w-full bg-white  text-black shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 border-t border-slate-100">
                <div className="max-w-7xl mx-auto px-4 py-10">
                  <div className="grid grid-cols-3 gap-8">
                    {/* Column 1 */}
                    <div>
                      <h3 className="font-bold text-orange-500 mb-4 uppercase text-xs tracking-widest">Support</h3>
                      <ul className="space-y-3 text-sm text-slate-600">
                        <li className="hover:text-orange-500 cursor-pointer transition">Help Center</li>
                        <li className="hover:text-orange-500 cursor-pointer transition">Order Tracking</li>
                        <li className="hover:text-orange-500 cursor-pointer transition">Returns & Exchanges</li>
                      </ul>
                    </div>

                    {/* Column 2 */}
                    <div>
                      <h3 className="font-bold text-orange-500 mb-4 uppercase text-xs tracking-widest">Contact</h3>
                      <ul className="space-y-3 text-sm text-slate-600">
                        <li className="hover:text-orange-500 cursor-pointer transition">Live Chat</li>
                        <li className="hover:text-orange-500 cursor-pointer transition">Email Us</li>
                      </ul>
                    </div>
                    <div>
                      <h3 className="font-bold text-orange-500 mb-4 uppercase text-xs tracking-widest">Support</h3>
